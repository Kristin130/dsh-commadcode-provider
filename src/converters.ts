/**
 * Message/tool conversion from the Harness request vocabulary to the Command
 * Code wire format, plus the SSE-line parser and finish-reason mapping.
 * Ported from pi-commandcode-provider's `src/converters.ts`; the auth-file
 * fallback is preserved so existing Command Code credentials keep working.
 *
 * @module dsh-commandcode-provider/converters
 */
...
/**
 * Resolve a Command Code API key from the environment (`COMMANDCODE_API_KEY`)
 * or an existing auth file. Kept as a fallback behind the Harness credential
 * seam, matching the pi plugin's file support:
 *
 * - `~/.commandcode/auth.json`
 * - `~/.pi/agent/auth.json`
 * - `~/.omp/agent/auth.json`
 */
export function getApiKey(options: GetApiKeyOptions = {}): string | undefined {
...
/** Parse one SSE-style line of the Command Code response stream. */
