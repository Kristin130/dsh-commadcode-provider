 ...
 *
 * Starts a one-shot server on a CLI-compatible localhost port. The Command
 * Code Studio website POSTs the user's API key to /callback after they
 * authenticate. Ported from pi-commandcode-provider's `src/auth-server.ts`.
 *
 * @module dsh-commandcode-provider/auth-server
 */
...
export async function startAuthServer(options: AuthServerOptions = {}): Promise<AuthServer> {
  ...
  const server = createServer((req, res) => {
    ...
    const responseOrigin = origin !== '' && allowedOrigins.includes(origin) ? origin : 'http://localhost:3000'
